    ...
    function create_regular_product($products, $dom) {
      ...
      if (STOCK_CHECK == 'true') {
        if ($products->fields['products_quantity'] > 0) {
          $item->appendChild($dom->createElement('g:quantity', $products->fields['products_quantity']));
          $item->appendChild($dom->createElement('g:availability', 'in stock'));
        } else {
          $item->appendChild($dom->createElement('g:quantity', (int)GOOGLE_PRODUCTS_DEFAULT_QUANTITY));
        ...
        if (GOOGLE_PRODUCTS_IN_STOCK == 'true') {
          if ($products->fields['products_quantity'] > 0) {
            $item->appendChild($dom->createElement('g:quantity', $products->fields['products_quantity']));
          } else {
            $item->appendChild($dom->createElement('g:quantity', GOOGLE_PRODUCTS_DEFAULT_QUANTITY));
          }
        }
        $item->appendChild($dom->createElement('g:availability', 'in stock'));                  
      }
      if(GOOGLE_PRODUCTS_WEIGHT == 'true' && $products->fields['products_weight'] != '') {
        $item->appendChild($dom->createElement('g:weight', $products->fields['products_weight'] . ' ' . GOOGLE_PRODUCTS_UNITS));
      } 
    ...
    function google_base_image_url($products_image) {
      ...
      if (defined('GOOGLE_PRODUCTS_ALTERNATE_IMAGE_URL') && GOOGLE_PRODUCTS_ALTERNATE_IMAGE_URL != '') {
        if (strpos(GOOGLE_PRODUCTS_ALTERNATE_IMAGE_URL, HTTP_SERVER . '/' . DIR_WS_IMAGES) !== false) {
          ...
          return GOOGLE_PRODUCTS_ALTERNATE_IMAGE_URL . urlencode($products_image);
        } 
      }
      ...
      if ((function_exists('handle_image')) && (GOOGLE_PRODUCTS_IMAGE_HANDLER == 'true')) {
        ...
        $retval = (HTTP_SERVER . DIR_WS_CATALOG . urlencode($products_image_large));
      }   
...
// SHIPPING FUNCTIONS //
  ...
  function shipping_rate($method, $percategory='', $freerules='', $table_zone = '', $products_weight = '', $products_price = '', $products_id = '') {
    ...
    //if ($rate >= 0) {
       //$rate = $currencies->format($rate, true, GOOGLE_PRODUCTS_CURRENCY, $currencies->get_value(GOOGLE_PRODUCTS_CURRENCY));
    //}
    return $rate;
  }
