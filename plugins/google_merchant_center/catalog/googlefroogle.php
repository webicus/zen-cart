          ...
          if ($google_base->check_product($products->fields['products_id'])) {
            ...
            if ((strlen($products_description) >= 15)) {
                      ...
                      if ($variant == false) {
                        ...
                        if (STOCK_CHECK == 'true') {
                          if ($variants_quantity > 0) {
                            $item->appendChild($dom->createElement('g:quantity', $variants_quantity));
                        ...
                        if(GOOGLE_PRODUCTS_WEIGHT == 'true' && $variants_weight != '') {
                          $item->appendChild($dom->createElement('g:weight', $variants_weight . ' ' . GOOGLE_PRODUCTS_UNITS));
                        }                         
              ...
              // if product doesn't have attributes, create a regular item without attributes
                ...
                // add universal elements/attributes to products                                         
                $item = $google_base->universal_attributes($products, $item, $dom);
                if (GOOGLE_PRODUCTS_DEFAULT_PRODUCT_CATEGORY != '') {
                  $google_product_category = $dom->createElement('g:google_product_category');
                  $google_product_category->appendChild($dom->createCDATASection($default_google_product_category));
                  $item->appendChild($google_product_category);
                }
